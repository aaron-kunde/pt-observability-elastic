...
In Kibana ~trace.id~ and ~trace.parent~ can be verified under /Observability/ -> /APM/ -> /Services/ -> /app1/ -> /Transactions/ -> /AppRestController#app1/:
#+begin_example
  ...
  http
...
Difference between ~management.endpoints.web.exposure.include=info,health,metrics,prometheus~ and ~management.endpoints.web.exposure.include=*~:
#+begin_example
...
In Kibana ~trace.id~ and ~trace.parent~ can be verified under /Observability/ -> /APM/ -> /Services/ -> /app2m/ -> /Transactions/ -> /AppRestController#app2m/
...
Metrics are not collected as noted in:
#+begin_example
  [otel.javaagent 2024-08-23 14:31:43:825 +0000] [http-nio-8083-exec-2] WARN io.opentelemetry.javaagent.shaded.instrumentation.micrometer.v1_5.OpenTelemetryMeterRegistry - OpenTelemetry metrics bridge does not support reading measurements
#+end_example
...
| Field       |                            Value |
|-------------+----------------------------------|
| span_id     |                 4bb2f8def0778182 |
| trace_flags |                               01 |
| trace_id    | 01010000000000000000000000000004 |
...
**** TODO Logs
Logs are structured, like in App2. They have no ~trace.id~. They are not visible in APM, since no APM connection exists and no data is in the ~apm~ indices.
| Field              | Value                                  |
|--------------------+----------------------------------------|
| @timestamp         | 2024-08-22T09:37:34.762Z               |
| agent.ephemeral_id | af3dd5a0-c249-43df-850c-4c88505362ea   |
| agent.hostname     | 6457fb11cb1a                           |
| agent.id           | 9df9aa1c-f4c7-45f9-920f-0a4bc1b2afd2   |
| agent.name         | 6457fb11cb1a                           |
...
| input.type         | filestream                             |
| log.file.device_id | 2080                                   |
| log.file.inode     | 6869471                                |
| log.file.path      | /var/log/app_logs/app4.log.json        |
| log.level          | info                                   |
| log.offset         | 0                                      |
| message            | Starting app4. Listening on port: 8084 |
| service.name       | app4
