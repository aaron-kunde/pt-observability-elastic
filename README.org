...
- Calls to metrics are monitored in APM as well, which should not be done
...
#+end_src
Calls to metrics are monitored as well, which should not be done.
...
- Metrics are implemented, using [[https://prometheus.io/docs/guides/go-application/][Prometheus instrumentation]] and additional sent via OTLP
...
- Instrumentation of HTTP Requests with [[https://pkg.go.dev/go.opentelemetry.io/contrib/instrumentation/net/http/otelhttp][instrumentation library for net/http]]
- Instrumentation of DB Queries with [[https://github.com/go-gorm/opentelemetry][Opentelemetry for gorm]]
- Instrumentation of Kafka messages based on [[https://github.com/jurabek/otelkafka][OpenTelemetry Go Instrumentation for confluent-kafka-go]]
- Added trace ids to logs with context passing
...
**** Traces
- Traces are set in HTTP-and DB-Requests through instrumentation libraries
- Traces are set in Kafka-Messages through reimplementation of intstrumentation libraries:
  - ~traceparent~ in message headers
  - Propagation for Producer and consumer
- Individual traces are set in application, using ~trace.id~
- Everything is traced in this implementation, even the calls to prometheus, which should be excluded.
...
| Field              | Value                                 |
|--------------------+---------------------------------------|
| _id                | UvdfEJIBbet0FzGuKO_R                  |
| _index             | .ds-filebeat-8.11.1-2024.09.20-000001 |
| _score             | 0                                     |
| @timestamp         | Sep 20, 2024 @ 18:58:38.979           |
| agent.ephemeral_id | d0a4d67c-2706-4384-b741-4aa62f555e9c  |
| agent.hostname     | cc2ca5ce39f9                          |
| agent.id           | 26d1f1e4-6d3a-437b-8a66-27e26b1426b1  |
| agent.name         | cc2ca5ce39f9                          |
| agent.type         | filebeat                              |
| agent.version      | 8.11.1                                |
| ecs.version        | 1.6.0                                 |
| event.dataset      | app4m                                 |
| host.name          | cc2ca5ce39f9                          |
| input.type         | filestream                            |
| log.file.device_id | 2080                                  |
| log.file.inode     | 6817023                               |
| log.file.path      | /var/log/app_logs/app4m.log.json      |
| log.level          | info                                  |
| log.offset         | 1,888                                 |
| message            | Send data to topic topic4: 2          |
| service.name       | app4m                                 |
| trace.id           | 01050000000000000000000000000001
...
#+begin_src json
  ...
  "_score": 0,
  "_source": {
    "@timestamp": "2024-09-20T16:58:38.979Z",
    "ecs": {
      "version": "1.6.0"
    },
    "event": {
    ...
    "agent": {
      "id": "26d1f1e4-6d3a-437b-8a66-27e26b1426b1",
      "name": "cc2ca5ce39f9",
      "type": "filebeat",
      "version": "8.11.1",
      "ephemeral_id": "d0a4d67c-2706-4384-b741-4aa62f555e9c"
    },
    "service": {
      "name": "app4m"
    },
    ...
    "trace": {
      "id": "01050000000000000000000000000001"
    },
    "message": "Send data to topic topic4: 2"
  },
  "fields": {
    ...
    "message": [
      "Send data to topic topic4: 2"
    ],
    "agent.type": [
      "filebeat"
    ],
    "trace.id": [
      "01050000000000000000000000000001"
    ],
    "@timestamp": [
      "2024-09-20T16:58:38.979Z"
    ],
    "agent.id": [
      "26d1f1e4-6d3a-437b-8a66-27e26b1426b1"
    ],
    "ecs.version": [
      "1.6.0"
    ],
    ...
    "agent.ephemeral_id": [
      "d0a4d67c-2706-4384-b741-4aa62f555e9c"
    ],
    "log.level": [
      "info"
    ],
...
If ~trace.id~ is added to the logs, it is visible in APM under the trace / transaction.
If ~service.name~ is added to the logs, it is visible in APM under the /Services/ /Logs/ tab
...
The logs are visible in APM, if those fields are set and APM is used.
