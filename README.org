...
**** Metrics
Metrics are also available at the Prometheus endpoint, e.g. for a Go standard metric:
#+begin_src sh
  curl -s localhost:8184/actuator/prometheus | grep -e go_
#+end_src
