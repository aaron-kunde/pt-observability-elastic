...
#+end_example
Since these metrics are not presented all at once, they can't be analysed in Kibana out of the box.
...
Traces and spans are even possible with Kafka topcis on the consumer site.

**** Logs
Logs are not seen under APM
...
**** Metrics
Metrics are partially handled like in App1, but custom metrics are not transferred to Elasticsearch and therefore not visible in Kibana.

**** Traces
Traces have less information as in App1 or App2 (e.g. only ~http.request.method~ and ~http.response.status_code~, but contain also ~parent.id~ and ~trace.id~.

**** Logs
Logs are structured and can be seen under APM, like in App2.
